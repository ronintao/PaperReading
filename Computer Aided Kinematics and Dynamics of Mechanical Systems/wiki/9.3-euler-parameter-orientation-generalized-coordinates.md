...
- 转轴称为**朝向轴（orientation axis）**，用单位向量 $\mathbf{u}$ 标记（右手规则，$\chi>0$ 逆时针）。
- 若把 $\mathbf{u}$ 取反（$\mathbf{u}\to-\mathbf{u}$），则原来的 $\chi$ 变为 $2\pi-\chi$——**同一朝向可由 $(\mathbf{u},\chi)$ 与 $(-\mathbf{u},2\pi-\chi)$ 两种方式实现**。这一二值性稍后会体现在 Euler 参数的 $\pm\mathbf{p}$ 上。
...
在图 9.3.3 中，把 $\mathbf{k}$ 绕 $\mathbf{u}$ 转 $\chi$ 得到 $\mathbf{h}$。构造：
- **沿 $\mathbf{u}$ 的分量**：$\mathbf{k}$ 在 $\mathbf{u}$ 方向的投影 $(\mathbf{u}^T\mathbf{k})\mathbf{u}$；旋转不改变它，故 $\mathbf{h}$ 中的 $\mathbf{u}$ 分量也是 $(\mathbf{u}^T\mathbf{k})\mathbf{u}$，端点记为 $N$。
- **在与 $\mathbf{u}$ 垂直的平面里**，$\mathbf{k}-(\mathbf{u}^T\mathbf{k})\mathbf{u}$ 是从 $N$ 到 $\mathbf{k}$ 尖端的向量。以 $N$ 为圆心、以 $|\mathbf{k}-(\mathbf{u}^T\mathbf{k})\mathbf{u}|$ 为半径的圆内，$\mathbf{k}$ 绕 $\mathbf{u}$ 转 $\chi$ 到 $\mathbf{h}$。
- **两个正交基**：与 $\mathbf{u}$ 及 $\mathbf{k}-(\mathbf{u}^T\mathbf{k})\mathbf{u}$ 共面且垂直的方向是 $\tilde{\mathbf{u}}\mathbf{k}$（因为 $\tilde{\mathbf{u}}\mathbf{k}$ 与 $\mathbf{u}$、$\mathbf{k}$ 都垂直，等价于 $\mathbf{u}\times\mathbf{k}$）；长度恰为该圆半径（$|\mathbf{u}\times\mathbf{k}|=|\mathbf{k}|\sin\theta=|\mathbf{k}-(\mathbf{u}^T\mathbf{k})\mathbf{u}|$）。

于是圆内向量 $\mathbf{a}$（沿 $\tilde{\mathbf{u}}\mathbf{k}$ 方向）与 $\mathbf{b}$（沿 $\mathbf{k}-(\mathbf{u}^T\mathbf{k})\mathbf{u}$ 方向）分别为：

$$
\mathbf{a}=\tilde{\mathbf{u}}\mathbf{k}\sin\chi,\qquad \mathbf{b}=[\mathbf{k}-(\mathbf{u}^T\mathbf{k})\mathbf{u}]\cos\chi
$$
