...
- 第三项 $2\tilde{\mathbf{u}}\mathbf{k}\sin(\chi/2)\cos(\chi/2)=2\bigl(\widetilde{\mathbf{u}\sin\tfrac{\chi}{2}}\bigr)\mathbf{k}\,\cos\tfrac{\chi}{2}=2e_0\tilde{\mathbf{e}}\mathbf{k}$。
...
类似的，由于 $\mathbf{f}\text{-}\mathbf{i}$、$\mathbf{g}\text{-}\mathbf{j}$ 与 $\mathbf{h}\text{-}\mathbf{k}$ 是完全同构的几何关系（都是"某轴单位向量绕 $\mathbf{u}$ 转 $\chi$"），把 $\mathbf{k}$ 换为 $\mathbf{i},\mathbf{j}$ 得到

$$
\mathbf{f}=[(2e_0^2-1)\mathbf{I}+2(\mathbf{e}\mathbf{e}^T+e_0\tilde{\mathbf{e}})]\mathbf{i},\quad \mathbf{g}=[(2e_0^2-1)\mathbf{I}+2(\mathbf{e}\mathbf{e}^T+e_0\tilde{\mathbf{e}})]\mathbf{j}
$$

因此，两个坐标系之间的转换关系为：

$$
[\mathbf{f},\mathbf{g},\mathbf{h}]=[(2e_0^2-1)\mathbf{I}+2(\mathbf{e}\mathbf{e}^T+e_0\tilde{\mathbf{e}})]\,[\mathbf{i},\mathbf{j},\mathbf{k}]
$$
...
这就是**用 Euler 参数写出的方向余弦矩阵**。

---

## 二、从 $\mathbf{A}$ 求解 Euler 参数
...
由 $\mathbf{u}$ 是单位向量，$\mathbf{u}^T\mathbf{u}=1$ 得到：
...
$$
\operatorname{tr}\mathbf{A}\equiv a_{11}+a_{22}+a_{33}
$$

对 (9.3.6) 表示的 $\mathbf{A}$ 求迹得到：

$$
\operatorname{tr}\mathbf{A}=3(2e_0^2-1)+2(e_1^2+e_2^2+e_3^2)=2(3e_0^2+e_1^2+e_2^2+e_3^2)-3=4e_0^2-1
$$
...
$$
\boxed{\;e_0^2=\tfrac{\operatorname{tr}\mathbf{A}+1}{4}\;}
$$

由 $a_{11}=2(e_0^2+e_1^2)-1$ 得到：

$$
e_1^2=\tfrac{a_{11}+1}{2}-\tfrac{\operatorname{tr}\mathbf{A}+1}{4}=\tfrac{2(a_{11}+1)-(\operatorname{tr}\mathbf{A}+1)}{4}=\tfrac{1+2a_{11}-\operatorname{tr}\mathbf{A}}{4}
$$

同理

$$
e_2^2=\tfrac{1+2a_{22}-\operatorname{tr}\mathbf{A}}{4},\qquad e_3^2=\tfrac{1+2a_{33}-\operatorname{tr}\mathbf{A}}{4}
$$

即，**只用对角元** 就能算出 4 个 Euler 参数的**模**。
...
$$
\boxed{\;e_1=\tfrac{a_{32}-a_{23}}{4e_0},\quad e_2=\tfrac{a_{13}-a_{31}}{4e_0},\quad e_3=\tfrac{a_{21}-a_{12}}{4e_0}\;}
$$

若 $e_0=0$，此时改用**对称加**：

$$
a_{21}+a_{12}=4e_1e_2,\quad a_{31}+a_{13}=4e_1e_3,\quad a_{32}+a_{23}=4e_2e_3
$$

至少有一个 $e_i\neq 0$（不然不满足归一化约束），任选其符号，其他两个的符号由上式确定。

#### - 双值性：$\mathbf{p}$ 与 $-\mathbf{p}$ 描述同一朝向

如果把 $e_0$ 的符号取反，则 $e_1,e_2,e_3$ 也会全部取反。但注意到 (9.3.7) 中每个元素都是 Euler 参数的二次多项式，所以 **$\mathbf{p}$ 和 $-\mathbf{p}$ 对应的$\mathbf{A}$ 矩阵是相同的**。这两者描述的是完全相同的朝向。

那在实际计算过程中，如何确定 $\mathbf{p}$ 的符号？只要在 $t=0$ 选定 $\mathbf{p}(0)$ 的符号，$\mathbf{p}(t)$ 的符号就由**连续性**唯一决定，运行过程中不会跳到镜像支。

**Euler 角在万向锁附近发散，Euler 参数处处光滑**。

---

## 三、旋转矩阵 $\mathbf{A}$ 的分解 —— $\mathbf{E},\mathbf{G}$ 矩阵
...
**两个 $3\times 4$ 矩阵，元素都是 Euler 参数。**

### 性质

容易证明 $\mathbf{E}$ 与 $\mathbf{G}$ 具有以下性质：

$\mathbf{E}$ 与 $\mathbf{p}$ 正交：

$$
\boxed{\;\mathbf{E}\mathbf{p}=\mathbf{0},\qquad \mathbf{G}\mathbf{p}=\mathbf{0}\;}\tag{9.3.20,\,9.3.21}
$$

**$\mathbf{E}$ 与 $\mathbf{G}$ 的行是正交单位向量组**（$\mathbf{E},\mathbf{G}$ 不是方阵，$\mathbf{E}^T\mathbf{E}\ne\mathbf{I}_4$）：
...
化简：$\mathbf{e}^T\mathbf{e}=1-e_0^2$；$\mathbf{e}^T\tilde{\mathbf{e}}=\mathbf{0}^T$（因 $\tilde{\mathbf{e}}^T\mathbf{e}=-\tilde{\mathbf{e}}\mathbf{e}=\mathbf{0}$），故 $-\mathbf{e}^T(\tilde{\mathbf{e}}+e_0\mathbf{I})=-e_0\mathbf{e}^T$；

右下 $3\times 3$（注意到根据 Eq 9.1.28，有 $\tilde{\mathbf{e}}\tilde{\mathbf{e}}=\mathbf{e}\mathbf{e}^T-(\mathbf{e}^T\mathbf{e})\mathbf{I}=\mathbf{e}\mathbf{e}^T-(1-e_0^2)\mathbf{I}$）：

$$
(\tilde{\mathbf{e}}+e_0\mathbf{I})^T(\tilde{\mathbf{e}}+e_0\mathbf{I})=-\tilde{\mathbf{e}}\tilde{\mathbf{e}}+e_0^2\mathbf{I}+e_0(\tilde{\mathbf{e}}^T+\tilde{\mathbf{e}})=-\tilde{\mathbf{e}}^2+e_0^2\mathbf{I}=-\mathbf{e}\mathbf{e}^T+\mathbf{I}_3
$$
