...
数值积分的地基思想只有一句话：**用一个多项式 $P(t)$ 去逼近函数 $f(t)$**，使 $P$ 在若干点 $t_i$ 上与 $f$（也许还有 $f$ 的若干阶导数）相等，然后拿容易积分的 $P$ 代替难积分的 $f$。逼近多项式的构造方法有很多，各有利弊，本节用到两种：Taylor 多项式（单点、多阶导数）与 Newton 后向差分多项式（多点、函数值）。
...
其中 $f^{(i)}(t_0)$ 表示 $f$ 在 $t_0$ 处的第 $i$ 阶导数。这个 $k-1$ 次多项式在 $t_0$ 处与 $f$ 及其前 $k-1$ 阶导数完全吻合（Prob. 7.4.1）：
...
> 逐项验证 7.4.3 的机制：$T_k$ 中第 $j$ 项是 $\frac{f^{(j)}(t_0)}{j!}(t-t_0)^j$，对它求 $j$ 阶导在 $t=t_0$ 处得 $\frac{f^{(j)}(t_0)}{j!}\cdot j!=f^{(j)}(t_0)$；而所有低于 $j$ 次的项被求导 $j$ 次后为零，所有高于 $j$ 次的项含因子 $(t-t_0)$ 在 $t_0$ 处为零。故第 $j$ 阶导恰为 $f^{(j)}(t_0)$。
...
$e$ 的十位精度值为 $e=2.718281828$，故 $e^{1/2}=1.648721271$。Table 7.4.1 列出 $T_k(t)$ 与误差 $E_k(t)=T_k(t)-e^t$ 在 $t=\tfrac12$ 与 $t=1$ 处、$k$ 从 1 到 14 的值。
...
**结论**：Taylor 多项式可把邻近点的函数值逼近到任意精度；且当 $\lvert t-t_0\rvert$ 越小、保留的项越多（用越高阶导数）时精度越高。在 $t=0.5$ 处 $k=11$ 已达十位精度，而 $t=1$ 处要到 $k=14$——印证了"距离越近收敛越快"。

**Taylor 展开的局限**：实际中，一个函数在**多个** $t$ 处的值往往比它在**单个** $t$ 处的高阶导数更容易得到。所以 Taylor 展开不适合做通用积分算法；它的价值在于帮助理解其他插值多项式如何发展成积分算法，以及推导数值积分误差估计。

### 插值多项式（Newton 后向差分形式）

**思路**：既然多点函数值更易得，就构造一个 $k-1$ 次多项式 $P_k(t)$，让它在 $k$ 个不同点 $t_{n-k+1},\dots,t_{n-1},t_n$ 上与 $f$ 相等：
...
其中**连乘记号**（与求和记号 $\sum_{j=0}^{\ell}$ 类似）定义为
...
即：$0$ 阶差分是函数值本身 $\nabla^0 f_n=f_n$，高阶差分是"相邻低阶差分之差"，与导数的差商类比。读者可验证 7.4.4 对 $k=1,2,3$ 给出正确的 $f_i$（Prob. 7.4.2）。

#### 例 7.4.2：用 Newton 后向差分多项式逼近 $e^t$

> 用 Eq. 7.4.4 在 $t=1$ 附近逼近 $e^t$，取 $t_n=1$、$h=0.1$。

对 $k=1,2,\dots,10$，计算 $P_k(1.1)$ 与 $P_k(1.2)$，误差 $E_k(t)=P_k(t)-e^t$ 见 Table 7.4.2（节选）：
...
正如预期：**多项式阶数越高、$\lvert t-1\rvert$ 越小，逼近越好**（$t=1.1$ 列比 $t=1.2$ 列收敛更快）。

### 插值误差（Interpolation Error）

**思路**：解微分方程时，需要把定义在一组数据点上的插值多项式**推进**到另一组数据点——扔掉最左边的旧点、在右边加入新点，从而沿时间网格前进。Taylor 与 Newton 后向差分多项式的形式提示：两组网格上大部分数据是相同的，能否只补一个"修正项"？

对 Taylor 多项式，把逼近点集从 $f_0,\dots,f_0^{(k-1)}$ 增加到 $f_0,\dots,f_0^{(k)}$，Eq. 7.4.2 直接给出

$$
T_{k+1}(t)=T_k(t)+\frac{f_0^{(k)}}{k!}(t-t_0)^k
$$

即：升一阶只需加一个更高次的修正项。对 Newton 后向差分插值，要达到类似效果（数据点从 $k$ 增到 $k+1$）就要求

$$
P_{k+1}(t)=P_k(t)+R_{k+1}(t)
\tag{7.4.5}
$$

其中 $R_{k+1}(t)$ 必须是 $k$ 次多项式——加一个数据点等于给当前插值多项式加一个更高次的修正项。

**求 $R_{k+1}$**：把 Eq. 7.4.4 写到 $k+1$ 项，减去写到 $k$ 项的 $P_k$，多出来的正是 $i=k$ 那一项：

$$
R_{k+1}(t)=\frac{\nabla^k f_n}{k!\,h^k}\prod_{j=0}^{k-1}(t-t_{n-j})
\tag{7.4.6}
$$

**把差分换成导数**：文献 31、36、37 证明了后向差分与导数的关系

$$
\frac{\nabla^k f_k}{k!\,h^k}=\frac{f^{(k)}(\xi)}{k!}
\tag{7.4.7}
$$

对某个 $\xi\in[t_{n-k},t_n]$ 成立。**把 7.4.7 代入 7.4.6**，并假设 $f(t)\approx P_{k+1}(t)$（即用高一阶插值近似真值），就得到用 $P_k$ 插值 $f$ 的误差估计：
...
对某个 $\xi\in[t_{n-k},t_n]$。误差好坏取决于 $f^{(k)}$ 在区间 $[t_{n-k},t_n]$ 上的变化幅度。

> **误差估计的工程价值**：Newton 后向差分多项式的一个重要用途是"用过去的函数值向前外推时间"（如例 7.4.2）。Eq. 7.4.8 表明，若被逼近函数的 $k$ 阶导数增长很快，逼近误差就增大。**虽然 $f^{(k)}$ 未知，但 Eq. 7.4.8 右端的误差可用 Eq. 7.4.7 左端的后向差分近似**——而后向差分在计算中是能实时监控的。一旦误差估计报警，就减小步长 $h$ 来压低误差。这正是后面自适应步长控制的理论依据。
