...
驱动约束**显含时间**，为系统提供运动驱动力。假定驱动约束数恰等于系统自由度，且

$$nh + \text{DOF} = nc$$

因此式 3.6.1 与 3.6.2 合起来恰是 $nc$ 个方程、$nc$ 个广义坐标。装配成
...
隐函数定理只给存在性、不给构造法。求解式 3.6.3 这类非线性方程最常用**牛顿-拉夫森方法**（第 4 章详述）：从满足 3.6.3 的某时刻 $t$ 的构型估计 $\mathbf{q}^{(0)}$ 出发，第 $k$ 次迭代解出修正量 $\Delta\mathbf{q}^{(k)}$：
...
位置解出后，对约束方程求一次时间导，把速度问题化成一个**以同一雅可比为系数**的线性方程组。
...
后续修正已小于容限。**仅两次迭代**，约束违背即小于规定误差，体现牛顿-拉夫森的二阶收敛。最终 $\phi_2 = 6.1488$ rad（$6.1488-2\pi=-0.1344$，故 $\cos\phi_2=0.9910,\ \sin\phi_2=-0.1340$）。
...
逐行回代验证（看出三段如何共用同一系数矩阵）：

$$\dot\phi_1 = 0.5236 \quad(\text{第6行,驱动速度})$$
$$\dot x_1 = 0.8660\,\dot\phi_1 = -\sin\phi_1\,\dot\phi_1 = 0.8660\times0.5236 = 0.4534 \quad(\text{第1行})$$
$$\dot y_1 = 0.5\,\dot\phi_1 = \cos\phi_1\,\dot\phi_1 = 0.5\times0.5236 = 0.2618 \quad(\text{第2行})$$
$$\dot y_2 = 0 \quad(\text{第5行})$$
$$\dot\phi_2 = -\frac{\cos\phi_1\,\dot\phi_1}{\cos\phi_2} = -\frac{0.5\times0.5236}{0.9910} = -0.2642 \quad(\text{第4行,}\ \dot y_2=0)$$
$$\dot x_2 = 0.8660\,\dot\phi_1 + 0.1340\,\dot\phi_2 = 0.4534 + 0.1340\times(-0.2642) = 0.4180 \quad(\text{第3行})$$

即

$$\boxed{\dot{\mathbf{q}} = [0.4534,\ 0.2618,\ 0.5236,\ 0.4180,\ 0,\ -0.2642]^T}$$
