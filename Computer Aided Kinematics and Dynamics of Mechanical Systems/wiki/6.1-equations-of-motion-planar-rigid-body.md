...
**加速度**：对 Eq. 6.1.5 求一阶、二阶时间导数。注意 $\dot{\mathbf{B}}=\dot\phi\,\dfrac{\partial\mathbf{B}}{\partial\phi}=-\dot\phi\,\mathbf{A}$：
...
Eq. 6.1.11 中左端含 $\int_m\mathbf{s}'^P\,dm$ 的整个第二项全为零，方程简化为
...
于是左侧第二项被积函数 $=\ddot\phi\,\mathbf{s}'^{PT}\mathbf{s}'^P$。

### 极转动惯量、合力、合力矩

定义相对质心的**极转动惯量 $J$**（polar moment of inertia）：
