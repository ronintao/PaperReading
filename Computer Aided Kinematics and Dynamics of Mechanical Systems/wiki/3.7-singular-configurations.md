...
3.2~3.6 节都假定雅可比（Jacobian）$\boldsymbol{\Phi}_\mathbf{q}$ **非奇异**——位置、速度、加速度三段分析才成立，隐函数定理（implicit function theorem）才保证唯一光滑解。本节专门研究**这个假定失效**的时刻：**奇异构型（singular configuration）**。运动可能在此处**走不下去（锁死）**，或者**分岔成多条路径**。主线逻辑：
...
**锁死（$\ell<1$）。** 曲柄在正 $\phi_1$ 方向被驱动。随运动推进，系统到达 $\sin\phi_1=\ell$ 的构型（图 3.7.1）。此处连杆与曲柄"撑直/对齐"，**物理上曲柄无法再沿正 $\phi_1$ 方向转动**——这就是锁死。

**分岔（$\ell=1$）。** 改取 $\ell=1$。当 $\phi_1=\pi/2,\ \phi_2=0$（图 3.7.2），曲柄沿正向继续驱动时有**两种**后续运动：

- 物体 2 停在 $O$ 处，物体 1、2 锁在一起像一个绕 $O$ 的单摆（pendulum）摆动 [图 3.7.3(a)]；
- 物体 2 越过 $O$ 点，沿负 $x$ 轴运动 [图 3.7.3(b)]。

这种"运动分叉成两条路径"即分岔。

![图 3.7.3 曲柄滑块的分岔](figures/computer-aided-kinematics-and-dynamics/fig_3_7_3_bifurcation_slidercrank.png)

还有一个有趣观察：在图 3.7.1 构型，若改为沿**负** $\phi_1$ 方向驱动，则点 $O_2$ 既可左移也可右移——同样是分岔。

---

## 例 3.7.2：平行四边形四连杆的分岔（直观）
...
这就把"病态运动"完全归结到"雅可比奇异"，可系统地数值检测。下面进一步区分这两种病态。
...
| 编号 | 含义 |
|------|------|
| (3.7.1) | 两体曲柄滑块的单坐标约束 $\Phi(q,t)=(q-\cos\omega t)^2+\sin^2\omega t-\ell^2=0$ |
| (3.7.2) | 其闭式解 $q=\cos\omega t\mp\sqrt{\cos^2\omega t+(\ell^2-1)}$，判别式定锁死/分岔 |
| (3.7.3) | 速度关系 $\Phi_q\dot q=2(q-\cos\omega t)\dot q=-2\omega q\sin\omega t=-\Phi_t$ |
| (3.7.4) | 三坐标曲柄滑块约束 $\boldsymbol{\Phi}(\mathbf{q},t)=\mathbf{0}$，$\mathbf{q}=[\phi_1,x_2,\phi_2]^T$ |
| (3.7.5) | 雅可比行列式 $|\boldsymbol{\Phi}_\mathbf{q}|=-\ell\sin\phi_2$，$\phi_2=0$ 时奇异 |
| (3.7.6) | 速度方程 $\boldsymbol{\Phi}_\mathbf{q}\dot{\mathbf{q}}=-\boldsymbol{\Phi}_t$ |
| (3.7.7) | 择一定理相容条件 $\boldsymbol{\beta}^T\boldsymbol{\Phi}_t=0$ |
| (3.7.8) | 左零向量定义 $\boldsymbol{\Phi}_\mathbf{q}^T\boldsymbol{\beta}=\mathbf{0}$ |
| (3.7.9) | 虚位移一阶约束 $\boldsymbol{\Phi}_\mathbf{q}\delta\mathbf{q}=\mathbf{0}$ |
| (3.7.10) | 分岔双速度 $\boldsymbol{\Phi}_\mathbf{q}(\dot{\mathbf{q}}\pm\delta\mathbf{q})=-\boldsymbol{\Phi}_t$ |
| (3.7.11) | 含设计参数的约束 $\boldsymbol{\Phi}(\mathbf{q},t;\mathbf{b})=\mathbf{0}$ |
| (3.7.12) | 设计变更线性化 $\boldsymbol{\Phi}_\mathbf{q}\delta\mathbf{q}=-\boldsymbol{\Phi}_\mathbf{b}\delta\mathbf{b}$ |
| (3.7.13) | 五杆平行四边形约束 $\boldsymbol{\Phi}^K(\mathbf{q})=\mathbf{0}$ |
| (3.7.14) | 其雅可比 $\boldsymbol{\Phi}^K_\mathbf{q}$，秩为 2（第三条冗余） |
