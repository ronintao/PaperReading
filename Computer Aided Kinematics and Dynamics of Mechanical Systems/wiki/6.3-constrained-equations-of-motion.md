...
第 6.1 节给出了单个刚体的微分/变分运动方程，6.2 节引入了虚功与广义力。本节把这套变分方法**推广到由多个刚体、且体间存在运动学约束**的系统。核心思路是：
...
对系统中每个物体，第 6.1 节的微分运动方程 (6.1.18)、(6.1.19) 都成立，前提是把作用在该物体上的**所有**力（含约束反力）都计入。把各物体的单体变分方程 (6.2.5) 相加，就得到整个系统的变分运动方程。

### 系统变分方程

$$\sum_{i=1}^{nb}\delta\mathbf{q}_i^T[\mathbf{M}_i\ddot{\mathbf{q}}_i - \mathbf{Q}_i] = 0 \tag{6.3.1}$$

对任意 $\delta\mathbf{q}_i$（$i=1,\dots,nb$）成立，前提是所有作用力都包含在 $\mathbf{Q}$ 中。

为方便处理多体系统，定义复合状态向量、复合（块对角）质量矩阵、复合广义力向量：
...
无论是单体方程 (6.2.5) 还是系统方程 (6.3.1)，广义力 $\mathbf{Q}$ 都必须同时含**约束力**和**外加力**。这里把"外加力"（applied force）定义为：除约束力之外，作用于系统上或体间的一切力——例如重力、弹簧-阻尼-作动器力都算外加力。
...
虽然作用在**单个**物体上的约束力虚功 $\delta\mathbf{q}_i^T\mathbf{Q}_i^C$ 一般不为零，但全系统约束力的**总**虚功为：

$$\sum_{i=1}^{nb}\delta\mathbf{q}_i^T\mathbf{Q}_i^C = \delta\mathbf{q}^T\mathbf{Q}^C$$

由牛顿第三定律：若运动学铰链无摩擦，约束力垂直于接触面、成对等大反向。因此，**只要把注意力限制在与约束相容的虚位移上**，所有约束力的总虚功为零：

$$\sum_{i=1}^{nb}\delta\mathbf{q}_i^T\mathbf{Q}_i^C = \delta\mathbf{q}^T\mathbf{Q}^C = 0$$

值得强调：即便对约束相容的虚位移，单体的 $\delta\mathbf{q}_i^T\mathbf{Q}_i^C \neq 0$；只有把各体方程**相加**成系统方程 (6.3.1) 后，约束力的影响才被整体抵消。这正是从单体方程上升到系统方程的价值所在。
...
注意：它们只对**与约束相容**的虚位移 $\delta\mathbf{q}$ 成立——这正是后面要用拉格朗日乘子处理的关键约束条件。
...
（具体约束方程形式见第 3 章。）虚位移 $\delta\mathbf{q}$ 是在**时间固定**下发生的。对 (6.3.4) 取微分并令 $\delta t = 0$：
...
其中雅可比 $\boldsymbol{\Phi}_{\mathbf{q}}$ 在满足 (6.3.4) 的状态 $\mathbf{q}$ 处取值。

**小结**：约束变分运动方程 = (6.3.3) 对所有满足 (6.3.5) 的 $\delta\mathbf{q}$ 成立。
...
$$\delta W = [\delta x_1, \delta y_1]^T\begin{bmatrix} 0 \\ -mg \end{bmatrix}
= [\delta x_1, \delta y_1, \delta\phi_1]^T\begin{bmatrix} 0 \\ -mg \\ 0 \end{bmatrix} = -mg\,\delta y_1$$
...
约束方程（铰链把摆固定在原点，质心距铰链 $\ell$）：

$$\boldsymbol{\Phi}(\mathbf{q}) = \begin{bmatrix} x_1 - \ell\cos\phi_1 \\ y_1 - \ell\sin\phi_1 \end{bmatrix} = \mathbf{0} \tag{6.3.7}$$
...
$$\boldsymbol{\Phi}(\mathbf{q}) = \begin{bmatrix}
x_1 - \ell_1\cos\phi_1 \\
y_1 - \ell_1\sin\phi_1 \\
...
几何含义：原本 (6.3.12) 只在子空间 $\{\mathbf{x}:\mathbf{A}\mathbf{x}=0\}$ 上成立；引入 $\boldsymbol{\lambda}$ 后等价于要求 $\mathbf{b}+\mathbf{A}^T\boldsymbol{\lambda}=\mathbf{0}$，即 $\mathbf{b}$ 落在 $\mathbf{A}$ 的行空间内。需注意定理**不**断言"对任意 $\mathbf{b},\mathbf{A}$ 只要 $\mathbf{A}\mathbf{x}=0$ 就有 $\mathbf{b}^T\mathbf{x}=0$"——前提要求 $\mathbf{b}$ 与 $\mathbf{A}$ 之间有此种紧密关系（见习题 6.3.2）。
...
> 合并细节：$\boldsymbol{\lambda}^T\boldsymbol{\Phi}_{\mathbf{q}}\delta\mathbf{q}$ 是标量，等于自身转置 $(\boldsymbol{\Phi}_{\mathbf{q}}^T\boldsymbol{\lambda})^T\delta\mathbf{q}$，故能与第一项合并写成 $[\,\cdot\,]^T\delta\mathbf{q}$。
...
> 验证：第一块行展开为 $\mathbf{M}\ddot{\mathbf{q}}+\boldsymbol{\Phi}_{\mathbf{q}}^T\boldsymbol{\lambda}=\mathbf{Q}^A$，即 (6.3.16)；第二块行展开为 $\boldsymbol{\Phi}_{\mathbf{q}}\ddot{\mathbf{q}}=\boldsymbol{\gamma}$，即 (6.3.17b)。
...
$$\ddot\Phi_1 = \ddot{x}_1 + \ell\cos\phi_1\,\dot\phi_1^2 + \ell\sin\phi_1\,\ddot\phi_1 = 0$$
$$\Rightarrow \underbrace{\ddot{x}_1 + \ell\sin\phi_1\,\ddot\phi_1}_{(\boldsymbol{\Phi}_{\mathbf{q}}\ddot{\mathbf{q}})_1} = -\ell\cos\phi_1\,\dot\phi_1^2$$
...
| 编号 | 含义 |
|------|------|
| (6.3.1)–(6.3.2) | 系统变分方程；复合 $\mathbf{q},\mathbf{M},\mathbf{Q}$ 记号 |
| (6.3.3) | 约束变分运动方程（仅对约束相容 $\delta\mathbf{q}$ 成立） |
| (6.3.4)–(6.3.5) | 约束方程；约束相容虚位移条件 $\boldsymbol{\Phi}_{\mathbf{q}}\delta\mathbf{q}=0$ |
| (6.3.6)–(6.3.8) | 例 6.3.1 单摆 |
| (6.3.9)–(6.3.11) | 例 6.3.2 曲柄滑块 |
| (6.3.12)–(6.3.14) | 拉格朗日乘子定理 |
| (6.3.15)–(6.3.16) | 乘子形式运动方程 $\mathbf{M}\ddot{\mathbf{q}}+\boldsymbol{\Phi}_{\mathbf{q}}^T\boldsymbol{\lambda}=\mathbf{Q}^A$ |
| (6.3.17) | 速度、加速度约束方程 |
| (6.3.18) | 混合 DAE 矩阵形式 |
| (6.3.19)–(6.3.20) | 例 6.3.4 单摆 DAE |
| (6.3.21)–(6.3.24) | 约束动力学存在定理及证明 |
| (6.3.25)–(6.3.30) | 初始位置/速度条件（含例 6.3.6） |
