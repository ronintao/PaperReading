...
为方便处理多刚体系统，定义系统的构型向量、质量矩阵、广义力向量：
...
这两式分别对任意 $\delta\mathbf{q}_i$、$\delta\mathbf{q}$ 成立（可参考 6.1 节中的重要中间结论及 Eq 6.1.18）。

### 关键：约束力的总虚功为零

虽然作用在**单个**物体上的约束力虚功 $\delta\mathbf{q}_i^T\mathbf{Q}_i^C$ 一般不为零。只要虚位移满足约束（称为相容），则由牛顿第三定律，约束力作用在被约束的两个物体上，垂直于接触面、成对、等大、反向。可以想见，所有约束力的总虚功为零。
...
注意：Eq 6.3.3 只对**与约束相容（consistent with the constraints）** 的虚位移 $\delta\mathbf{q}$ 成立。下面介绍如何找到这样的 $\delta\mathbf{q}$。
...
该方程求解得到的虚位移 $\delta\mathbf{q}$ 被称为是与约束相容的 (Consistent with the constraints / Kinematically Admissible virtual displacement)。

> Ronin 注：容易看到，“与约束相容”是满足“系统的约束力的总虚功为 0” 的必要条件。事实上，**是必要非充分条件**，使用这一必要条件是为了配合后续的拉格朗日乘子法。
> 在后续的完整约束方程组中，还需要去验证 Eq 6.3.4 的成立。


> Ronin 注2：**【重要注解 6.3】** 在求总虚功时，各个刚体可以独立选择自己的参考点来求取自己的力矩产生的虚功，然后再将虚功相加（如例 6.3.2）——甚至可以选择自己的坐标系来求取虚功，然后再相加。
> 这是因为，虚功是标量，且可以在 coordinate free 的情况下计算得到。因此容易证明其不受参考点选取——或是参考坐标系的选取——的变化而变化。
> 与之对应的，在经典刚体静力学中，同一刚体上受多个力，要统一参考点才能把力矩正确相加成合力矩。
...
广义坐标 $\mathbf{q} = [x_1, y_1, \phi_1]^T$。（注意是全局坐标，不是 $x_1'$）.重力为外加力，作用位置在质心上，因此力矩为0。因此广义外力为 $\mathbf{Q}^A = [0,\, -mg,\, 0]^T$。进而得到虚功为：

$$\delta W = [\delta x_1, \delta y_1, \delta\phi_1]^T\begin{bmatrix} 0 \\ -mg \\ 0 \end{bmatrix} = -mg\,\delta y_1$$

参考表 6.1.1，单摆绕质心的极转动惯量为 $m\ell^2/3$，质量矩阵 $\mathbf{M} = \operatorname{diag}(m, m, m\ell^2/3)$。由 (6.3.3)：
...
类似于上个例子，这里两个刚体各自选择了自己的质心作为力矩的参考点，得到各自受到的重力力矩为0。因此两刚体重力虚功：
...
(6.3.3) 只对约束相容的 $\delta\mathbf{q}$ 成立，难以直接得到运动方程（因为 $\delta\mathbf{q}$ 不自由）。经典做法是引入拉格朗日乘子，把它转化为对**任意** $\delta\mathbf{q}$ 都成立的代数方程。
...
因对所有 $\mathbf{x}$ 成立，三个系数各自为零，容易得到：

$$\boxed{\boldsymbol{\lambda} = [-1,\, -1]^T}$$

### 应用到动力学：运动方程的拉格朗日乘子形式

把定理对应到约束变分方程 Eq 6.3.3，以及约束条件 Eq 6.3.5：
- $\mathbf{b} \leftrightarrow \mathbf{M}\ddot{\mathbf{q}}-\mathbf{Q}^A$
- $\mathbf{x} \leftrightarrow \delta\mathbf{q}$
- $\mathbf{A} \leftrightarrow \boldsymbol{\Phi}_{\mathbf{q}}$

$[\mathbf{M}\ddot{\mathbf{q}}-\mathbf{Q}^A]^T\delta\mathbf{q}=0$ 对所有满足  $\boldsymbol{\Phi}_{\mathbf{q}}\,\delta\mathbf{q} = \mathbf{0} $ 的 $\delta\mathbf{q}$ 成立，满足定理前提。故存在乘子向量 $\boldsymbol{\lambda}$ 使下式对**任意** $\delta\mathbf{q}$ 成立：
...
由于 Eq 6.3.15 对任意 $\delta\mathbf{q}$ 成立，$\delta\mathbf{q}$ 前的系数必须为零向量，得到**运动方程的拉格朗日乘子形式**：
...
> Ronin 注：如 6.1 开始的重要注解所述，从第 6 章开始，尤其是例题中，只考虑了 Kinematical Constraints。拉格朗日乘子法和下面的 DAE 并不要求 $\boldsymbol{\Phi}$ 和 $\boldsymbol{\Phi}_{\mathbf{q}}$ 是方阵，因此对于 $\boldsymbol{\Phi}=\boldsymbol{\Phi}^K$ ，抑或是包含了一个或多个 $\boldsymbol{\Phi}^D$ 的约束，都是没有问题的。
...
这是一个**混合微分-代数方程组（DAE）**，方程中不包含乘子 $\boldsymbol{\lambda}$ 的任何导数。除 (6.3.18) 外，位置约束 (6.3.4) 和速度约束 (6.3.17a) 也必须始终满足。
...
对其求关于 $\mathbf{q}$ 的偏导，需要注意对 $\phi$ 求偏导时，$\dot\phi$ 被视作常量（已经被 $t$ 求导了），只有 $\sin\phi$ 被视作变量，其他变量也同理：

$$(\boldsymbol{\Phi}_{\mathbf{q}}\dot{\mathbf{q}})_{\mathbf{q}} = \begin{bmatrix} 0 & 0 & \ell\cos\phi_1\,\dot\phi_1 \\ 0 & 0 & \ell\sin\phi_1\,\dot\phi_1 \end{bmatrix}$$
