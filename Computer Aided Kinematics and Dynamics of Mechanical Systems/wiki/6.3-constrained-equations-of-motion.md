...
该方程求解得到的虚位移 $\delta\mathbf{q}$ 被称为是与约束相容的 **Consistent with the constraints** 或是 **Kinematically Admissible virtual displacement**。（注意，虽然包含 Kinematically 字样，但实际上同时要满足 kinematical constraints 和 driving constraints。不过如6.1开头所述，我们现在开始主要关注 kinematical constraints）。
...
广义坐标 $\mathbf{q} = [x_1, y_1, \phi_1]^T$。重力为外加力，作用位置在质心上，因此力矩为0。因此广义外力为 $\mathbf{Q}^A = [0,\, -mg,\, 0]^T$。进而得到虚功为：
